...
Number *Float::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Float* f = new Float(this->value + dynamic_cast<Integer*>(num)->val());
		delete num;

		return f;
	}
	else if (num->types() == FLOAT) {
		Float* f = new Float(this->value + dynamic_cast<Float*>(num)->val());
		delete num;

		return f;
	}
	else if (num->types() == COMPLEX) {
		Complex* c = new Complex(this->value + dynamic_cast<Complex*>(num)->val().first, dynamic_cast<Complex*>(num)->val().second);
		delete num;
...
Number *Float::mul(Number *num) {
	...
	if (num->types() == INTEGER) {
		...
		Complex* c = new Complex(this->value * dynamic_cast<Complex*>(num)->val().first, this->value * dynamic_cast<Complex*>(num)->val().second);
		delete num;
...
Number::type Float::types() const {
     ...
     */
    return Number::type(FLOAT); // 구현한 거 아님!
}
