...
Complex::Complex() {
     ...
     */
	this->set_val(std::make_pair(0,0));
}
...
Complex::Complex(float _val1, float _val2) {
     ...
     */
	this->set_val(std::make_pair(_val1, _val2));
}
...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		// 컴파일러에 의해 소숫점 밑 수는 자동으로 삭제될 것이다.
		Complex* c = new Complex(this->val().first + dynamic_cast<Integer*>(num)->val(), this->val().second);
		delete num;

		return c;
	}
	else if (num->types() == FLOAT) {
		Complex* c = new Complex(this->val().first + dynamic_cast<Float*>(num)->val(), this->val().second);
		delete num;

		return c;
	}
	else if (num->types() == COMPLEX) {
		Complex* c = new Complex(this->val().first + dynamic_cast<Complex*>(num)->val().first, this->val().second + dynamic_cast<Complex*>(num)->val().second);
		delete num;

		return c;
	}
	else {
		// 아무 작업도 하지 않는다. (nullptr 반환)
	}
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		// 컴파일러에 의해 소숫점 밑 수는 자동으로 삭제될 것이다.
		Complex* c = new Complex(this->val().first * dynamic_cast<Integer*>(num)->val(), this->val().second * dynamic_cast<Integer*>(num)->val());
		delete num;
		
		return c;
	}
	else if (num->types() == FLOAT) {
		Complex* c = new Complex(this->val().first * dynamic_cast<Float*>(num)->val(), this->val().second * dynamic_cast<Float*>(num)->val());
		delete num;
		...
		Complex* c = new Complex(this->val().first * dynamic_cast<Complex*>(num)->val().first - this->val().second * dynamic_cast<Complex*>(num)->val().second,
			this->val().first * dynamic_cast<Complex*>(num)->val().second + this->val().second * dynamic_cast<Complex*>(num)->val().first);
		delete num;
...
void Complex::set_val(double real, double imag) {
     ...
     * 이 때 make_pair함수를 이용
     */
	Complex::value.first = real;
	Complex::value.second = imag;
}
...
Number::type Complex::types() const {
     ...
     */
    return Complex::type(COMPLEX); // 구현한거 아님..!
}
